...
export class UyeComponent implements OnInit {
  ...
  Sil(uye: Uye, el: HTMLElement) {
   ...
   UyeListele() {
     this.fbservis.UyeListele().subscribe((d: Uye[]) => {
       this.uyeler = d;
     });
   }
  UyeEkleDuzenle() {
    ...
    if (!uye.uid) {
       ...
       if (filtre.length > 0) {
         ...
         uye.duztarih = tarih.getTime().toString();
          this.fbservis.UyeEkle(uye).subscribe((d: Uye[]) => {
      ...
      uye.duztarih = tarih.getTime().toString();
      this.fbservis.UyeDuzenle(uye).subscribe((d: Uye[]) => {
        this.sonuc.islem = true;
  ...
  UyeSil() {
    this.fbservis.UyeSil(this.secUye.uid).then((d: Uye[]) => {
      this.sonuc.islem = true;
