...
export class FbservisService {
  ...
  HaberListele() {
    ...
    return collectionData(ref, { idField: 'haberId' }) as Observable<Haber[]>;
  }
  HaberListeleByKatId(katId: string) {
    var ref = collection(this.fs, "Haberler");
    const myQuery = query(ref, where("katid", "==", katId));
    return collectionData(myQuery, { idField: "haberid" }) as Observable<Haber[]>
  }
  ...
  HaberDuzenle(Haber: Haber) {
    var ref = doc(this.fs, "Haberler/" + Haber.haberId);
